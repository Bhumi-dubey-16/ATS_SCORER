...
with c1:
    st.markdown(f"""
    <div class="stat-card">
        <div class="stat-eyebrow">Total Scans</div>
        <div class="stat-value">{len(history)}</div>
        <div class="stat-sub">resumes analysed</div>
    </div>""", unsafe_allow_html=True)

with c2:
    st.markdown(f"""
    <div class="stat-card">
        <div class="stat-eyebrow">Average Score</div>
...
with c3:
    st.markdown(f"""
    <div class="stat-card">
        <div class="stat-eyebrow">Best Score</div>
        <div class="stat-value" style="color:#10b981">{best_score}</div>
...
with c4:
    ...
    st.markdown(f"""
    <div class="stat-card">
        <div class="stat-eyebrow">Latest Trend</div>
        <div class="stat-value" style="color:{trend_color}">{trend_str}</div>
        <div class="stat-sub">vs previous scan</div>
    </div>""", unsafe_allow_html=True)
...
chart_data = list(reversed(history))  # oldest first
...
fig.update_layout(
    ...
    xaxis=dict(
        showgrid=False,
        zeroline=False,
        tickfont=dict(color="#4a4a60", size=11)
    ),
    yaxis=dict(
        showgrid=True,
        gridcolor="#1a1a2a",
